...
I have about 12 total square feet of grass on my property. I NEVER think about the lawn (ask my wife)... let alone maintaining it, and the proximity of this ad timing to the conversation just made it too coincidental. It also made me think of my Dad. So I couldn't drop it. 
...
Enter mitmproxy. Installed on the pi, it could unlock the payload of anything that came across the wire - assuming the device had a trusted "key".
...
The problem here was that every device on the network needed to have its own key installed - and many devices won't really let you. Phones make it hard, TVs and Xboxs make it impossible. As a result, again I chose to focus on my laptop, and leave the payloads from other devices indecipherable for now.

Even with that, there was still traffic I couldn't decrypt. WebRTC, which is **really** what you'd use if you were trying to send backdoor audio without being noticed, is encrypted end-to-end by design — mitmproxy can't touch it. And some connections use Certificate Pinning, which identifies exactly which "Key" it trusts, and won't let you use your own. 

<BLineImage
  photo
  src="https://assets.andrewheins.ca/images/audit-inspect-detail-screenshot.jpeg"
...
<BLinePanel id="full-architecture" title="Full Monitoring Architecture">
  ...
  <p><strong>Layer 4 — TLS interception:</strong> mitmproxy running on the Pi, with a self-signed root certificate trusted by the Mac. Decrypts most HTTPS traffic in transit. Defeated by certificate pinning and WebRTC, which are opaque by design.</p>
